...
public abstract class BaseCard implements Card {
    ...
    private void setName(String name) {
        if(name==null || name.trim().isEmpty()){
            throw new IllegalArgumentException(CARD_NAME_IS_NULL);
        }
        this.name = name;
    }
    ...
    public void setDamagePoints(int damagePoints) {
        if(damagePoints<MIN_DAMAGE_POINTS){
            throw new IllegalArgumentException(CARDS_DAMAGE_POINTS_ZERO);
        }
    ...
    private void setHealthPoints(int healthPoints) {
        if(healthPoints<MIN_HEALTH_POINTS){
            throw new IllegalArgumentException(CARDS_HEALTH_POINTS_ZERO);
        }
